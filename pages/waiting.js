...
export default function Waiting(props) {
  ...
  const positive = Interval.fromDateTimes(rawTimeNow, serverTime).length(
    "milliseconds"
  );
  const negative = -Interval.fromDateTimes(serverTime, rawTimeNow).length(
    "milliseconds"
  );
  ...
  const gameOver =
    !Interval.fromDateTimes(timeNow, startDate).length("seconds") && isToday;
  ...
  useEffect(() => {
    ...
    const timeout =
      Interval.fromDateTimes(timeNow, startDate).length("milliseconds") + 150;
